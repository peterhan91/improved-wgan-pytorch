...
DATA_DIR = '//nas5.pmi.rwth-aachen.de/mri-scratch/DeepLearning/CheXpert_Dataset/images_256/images/'
VAL_DIR = '//nas5.pmi.rwth-aachen.de/mri-scratch/DeepLearning/CheXpert_Dataset/images_256/images/'

IMAGE_DATA_SET = 'cxr' #change this to something else, e.g. 'imagenets' or 'raw' if your data is just a folder of raw images. 
...
OUTPUT_PATH = './runs/128x128/' # output path where result (.e.g drawing images, cost, chart) will be stored
...
OUTPUT_DIM = 128*128*1 # Number of pixels in each iamge
...
def load_data(path_to_folder, classes, fold=None):
    ...
    elif IMAGE_DATA_SET == 'cxr':
        dataset = PatchDataset(path_to_folder, fold, transform=data_transform)
    else:
...
def gen_rand_noise_with_label(label=None, cxr=False):
    if label is None:
...
if __name__ == '__main__':
    ...
    if RESTORE_MODE:
        ...
        aG = GoodGenerator(64, OUTPUT_DIM)
    ...
    def train():
        dataloader = load_data(DATA_DIR, None, fold='train')
        ...
        for iteration in range(START_ITER, END_ITER):
            ...
            for i in range(GENER_ITERS):
                ...
                noise = gen_rand_noise_with_label(real_label, cxr=True)
                noise.requires_grad_(True)
                fake_data = aG(noise)
                gen_cost, gen_aux_output = aD(fake_data)

                # aux_label = torch.from_numpy(f_label).long()
                aux_label = real_label.to(device).float()
                aux_errG = aux_criterion(torch.sigmoid(gen_aux_output), aux_label).mean()
            ...
            for i in range(CRITIC_ITERS):
                ...
                real_label = real_label.to(device).float()

                # train with real data
                disc_real, aux_output = aD(real_data)
                aux_errD_real = aux_criterion(torch.sigmoid(aux_output), real_label)
        ...
        #----------------------Generate images-----------------
            ...
            if iteration > 1 and iteration % 200==0:
                val_loader = load_data(VAL_DIR, None, fold='test')
                dev_disc_costs = []
        ...
        #----------------------Save model----------------------
                torch.save(aG, OUTPUT_PATH + "generator_"+str(iteration)+".pt")
                torch.save(aD, OUTPUT_PATH + "discriminator_"+str(iteration)+".pt")
